...
## 2. マッチング（スコアリング）— **プロンプトなし・決定論ロジック**
...
## 5. 【案】LLMマッチング判定プロンプト（編集してください）

> これは「マッチングをLLMで判定する」ための**たたき台**です。
> この節のテキストを自由に編集 → 確定したらコードに組み込みます。
> 方式は **一括ランキング**（1案件＋候補人材リストをまとめて判定）を想定。
