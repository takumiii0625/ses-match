...
【勤務地チェック】locationOk を必ず判定する:
- 案件の勤務地・常駐/リモート条件と、人材の所在地（最寄り駅）・リモート希望を突き合わせる。
- 次のような「明らかに両立しない」場合のみ locationOk=false（このマッチは除外される）:
  例: 案件が「常駐必須・東京」× 人材が「フルリモート希望・福岡在住で出社不可」→ false。
  例: 案件が「週3出社・大阪」× 人材が「遠方在住・出社不可」→ false。
- 次のように対応可能性がある場合は locationOk=true:
  例: 案件がフルリモート可、または 人材が通勤圏内、または 人材が「出社応相談/可」。
- 勤務地やリモート条件が不明で判断材料が足りない場合も locationOk=true（無理に除外しない）。
- 注意: 勤務地が両立しない場合は score を高くしない（提案優先度としても下げる）。
