...
export class AnthropicAIService implements AIService {
  ...
  /** 差し戻し分析の1エージェント（1観点）を実行し、抽出した箇条書きを返す。 */
  private async rejectionAgent(system: string, input: string): Promise<string> {
    const res = await this.client.messages.create({
      model: MODEL,
      max_tokens: 1000,
  ...
  /**
   * 差し戻し(送らない判断)の一覧から「避けるべきマッチ傾向」を抽出する。
   * SESの実務では「商流が引っかかって提案できない」ケースが最も多いため、複数の専門エージェント
   * （商流・契約形態／単価・粗利／勤務地・稼働／スキル・属性）で観点別に分析し、最後に統合する。
   * 統合結果はマッチ判定プロンプトへ自動付加され、該当マッチを提案不可/減点にする。
   */
  async analyzeRejections(input: string): Promise<string> {
    const COMMON = `以下は営業担当が「この人材×案件は送らない（差し戻し）」と判断したマッチと理由の一覧です（案件の商流制限・人材の所属も併記）。
あなたはSES営業歴の長いプロのマネージャです。担当の判断の背後にある「繰り返し現れる確かな傾向」だけを、今後のマッチ判定に使える形で抽出してください。

厳守ルール:
- 個別の固有名詞（会社名・人名）は使わず、一般化した条件で書く。
- 「〜の場合は提案不可（除外）にする／大きく減点する」など、判定に直接使える指示の形にする。
- 【重要】「既に充足」「他社で決定」「タイミングが合わない」「今回は見送り」等の“一回限り・状況依存”の理由は学習に含めない。属性ベースの再現する傾向だけを書く。
- 自分の担当観点に該当する傾向が無ければ「該当なし」とだけ書く。
- 箇条書き本文のみ出力（前置き・見出し・コードブロック不要）。`;

    // SES実務で最頻の「商流詰まり」を最重要エージェントとして厚く分析する。
    const AGENTS: { tag: string; system: string }[] = [
      {
        tag: "商流・契約形態",
        system: `${COMMON}

【あなたの担当＝商流・契約形態（最重要）】商流に起因する差し戻しを重点的に、具体的に抽出してください。次を必ず点検する:
- 商流の深さ超過（自社が仲介すると+1段深くなる点を踏まえた「自社視点N社先まで」の超過）。
- 「貴社/御社まで・貴社所属/社員/プロパー/要員/フリーランスまで」＝受信会社止まり案件に他社人材を当てている。
- 委託回数・N次請け制限の超過。
- 派遣契約必須（自社は派遣不可）・個人事業主/フリーランス不可・法人契約のみ、といった契約形態の不一致。
- 支援費ありで商流を飛ばせるはずが弾かれている等の運用ミス。
商流の数え方（自社視点で1段深くなる／貴社止まりは自社保有のみ可）を前提に、再現する条件だけを指示形で書く。`,
      },
      {
        tag: "単価・粗利",
        system: `${COMMON}

【あなたの担当＝単価・粗利】単価/粗利に起因する差し戻しを抽出してください。次を点検する:
- 逆ザヤ・粗利ゼロ（案件単価 ≤ 人材単価）で送っている他社人材。
- 案件予算上限を明確に超える希望単価。
- 単価が近接・レンジ重複・応相談なのに過剰に弾いている運用ミス（減点しすぎ）。
再現する条件だけを指示形で書く。`,
      },
      {
        tag: "勤務地・稼働",
...
【あなたの担当＝勤務地・勤務形態・稼働】次を点検する:
- 常駐/出社案件に対する居住地域の不一致（地方区分をまたぐ通勤不可）。
- 出社頻度の不整合（週N出社に対し人材が対応不可）。
- 「地方在住不可／首都圏のみ」案件への地方人材。
- 稼働開始時期の不一致。
再現する条件だけを指示形で書く。`,
      },
      {
...
【あなたの担当＝スキル・経験・年齢・国籍】次を点検する:
- 必須スキル/技術スタックの不足・別言語（大分類一致だけの誤マッチ）。
- 必須の経験年数不足。
- 年齢制限の超過。
- 外国籍不可（帰化含む）案件への外国籍人材。
再現する条件だけを指示形で書く。`,
      },
    ];
    ...
    const parts = await Promise.all(
      AGENTS.map(async (a) => {
        const body = await this.rejectionAgent(a.system, input);
        return { tag: a.tag, body };
      }),
    );
    const merged = parts
      .filter((p) => p.body && !/^該当なし$/.test(p.body.trim()))
      .map((p) => `【${p.tag}】\n${p.body}`)
      .join("\n\n");

    if (!merged.trim()) return "特筆すべき傾向なし";

    // 統合エージェント: 観点別の抽出を1本化し、SES実務の優先度順に並べ替え・重複排除する。
    const synthSystem = `あなたはSESマッチング精度改善の責任者です。以下は観点別の専門家が差し戻し履歴から抽出した「避けるべきマッチ傾向」です。
これらを統合し、今後のマッチ判定プロンプトに付加する最終版の箇条書きに仕上げてください。

ルール:
- 重複・言い換えは1項目に統合する。
- 実務上の優先度が高い順に並べる: ①商流 ②契約形態（派遣不可/個人事業主可否）③単価・粗利 ④勤務地・稼働 ⑤スキル・経験 ⑥年齢・国籍。
- 各項目は「〜の場合は提案不可（除外）／大きく減点する」の指示形で、判定に直接使えるよう具体的に書く。
- 最大12項目。抽象的・一回限りの内容は落とす。
- 箇条書き本文のみ出力（前置き・見出し・コードブロックは不要）。`;
    const res = await this.client.messages.create({
      model: MODEL,
      max_tokens: 1400,
      system: [{ type: "text", text: synthSystem }],
      messages: [{ role: "user", content: merged }],
    });
    logUsage("rejection-analysis", res.usage);
    const text = res.content.find((b) => b.type === "text");
    return text && text.type === "text" ? text.text.trim() : merged;
  }
