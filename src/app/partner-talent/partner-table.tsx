...
import { TalentDrawer, type TalentDrawerData } from "@/components/talent-drawer";

interface PartnerTableProps {
  talents: TalentDrawerData[];
  total: number;
}

export function PartnerTable({ talents, total }: PartnerTableProps) {
  const [selected, setSelected] = useState<Set<string>>(new Set());
  const [openTalent, setOpenTalent] = useState<TalentDrawerData | null>(null);
            ...
            {talents.length === 0 ? (
              ...
              talents.map((talent) => (
                ...
                >
                  ...
                  <td className="px-3 py-2.5 text-slate-500 font-mono text-xs whitespace-nowrap">{talent.managementId || "-"}</td>
                  <td className="px-3 py-2.5 whitespace-nowrap">
                    <Badge tone={statusTone(talent.status)}>
                      {TALENT_STATUS_LABELS[talent.status] ?? talent.status}
                    </Badge>
                  </td>
                  <td className="px-3 py-2.5 text-slate-700 whitespace-nowrap">{talent.assignee?.name ?? "-"}</td>
                  <td className="px-3 py-2.5 font-medium text-slate-800 whitespace-nowrap">{talent.name}</td>
