...
import { DeleteButton } from "@/app/projects/[id]/delete-button";

type ProjectWithDetail = Prisma.ProjectGetPayload<{
  include: {
    assignee: true;
    matches: { include: { talent: true } };
  };
}>;

/** 案件の詳細＋編集ビュー。/projects/[id] と /in-house-project/[id] で共有。 */
    ...
    <div className="flex flex-col gap-6 p-6 min-h-full">
      ...
      <Card className="p-5">
        <dl className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          ...
          <div>
            <dt className="text-xs text-slate-500 mb-0.5">エンド/クライアント</dt>
            <dd className="text-slate-800">{project.clientName ?? "-"}</dd>
          </div>
          <div>
            <dt className="text-xs text-slate-500 mb-0.5">商流制限</dt>
            <dd className="text-slate-800">{project.channelText ?? "-"}</dd>
          </div>
          <div>
